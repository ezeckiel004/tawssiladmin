    ...
    <div className="p-4 mb-6 rounded-lg bg-blue-50 border border-blue-200">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="mt-1 text-sm text-blue-700">
            ...
            {status === 'en_attente' && (
              <span className="block mt-1 font-medium">
                ⚡ Vous pouvez assigner directement un distributeur ou passer le colis en transit.
              </span>
            )}
            {status === 'en_transit' && (
              <span className="block mt-1 font-medium">
          ...
          {isAdmin && status === 'en_attente' && (
            <button
              onClick={onPasserEnTransit}
              className="flex items-center gap-2 px-4 py-2 mt-3 text-white bg-blue-600 rounded-lg hover:bg-blue-700"
            >
              <FaRoute className="w-4 h-4" />
              Passer directement en transit
            </button>
          )}
        </div>
      </div>
...
const LivraisonDetail = () => {
  ...
  const handlePasserEnTransit = async () => {
    try {
      await livraisonService.passerEnTransit(id);
      toast.success("Livraison passée en transit avec succès");
      fetchLivraison();
    } catch (error) {
      toast.error(error.response?.data?.message || "Erreur lors du passage en transit");
    }
  };
  ...
  const handlePrint = async () => {
    ...
    try {
      ...
      if (response.success && response.html) {
        ...
        const printWindow = window.open("", "_blank", `width=${window.innerWidth},height=${window.innerHeight},scrollbars=yes,resizable=yes`);
      ...
      try {
        ...
        toast.error("Erreur lors de l'impression. Veuillez essayer le téléchargement PDF.");
      }
    } finally {
      setTimeout(() => {
        setIsPrinting(false);
  ...
  const getStatusInfo = (status) => {
    const statusConfig = {
      en_attente: { icon: FaClock, color: "text-yellow-600 bg-yellow-100", label: "En attente", description: "En attente de traitement" },
      prise_en_charge_ramassage: { icon: FaTruck, color: "text-blue-600 bg-blue-100", label: "Prise en charge ramassage", description: "Prise en charge par le ramasseur" },
      ramasse: { icon: FaBox, color: "text-purple-600 bg-purple-100", label: "Ramasse", description: "Colis ramassé avec succès" },
      en_transit: { icon: FaRoute, color: "text-indigo-600 bg-indigo-100", label: "En transit", description: "En route vers la destination" },
      prise_en_charge_livraison: { icon: FaTruck, color: "text-orange-600 bg-orange-100", label: "Prise en charge livraison", description: "Prise en charge par le livreur" },
      livre: { icon: FaCheckCircle, color: "text-green-600 bg-green-100", label: "Livré", description: "Livraison terminée" },
      annule: { icon: FaTimesCircle, color: "text-red-600 bg-red-100", label: "Annulé", description: "Livraison annulée" },
    };
  ...
  const canAssignLivreur = (livraison, type) => {
    ...
    if (type === "distributeur") {
      if (isDepot) {
        return ['en_attente', 'en_transit'].includes(livraison.status);
      }
      return livraison.status === "en_transit";
    } else if (type === "ramasseur") {
      if (isDepot) return false;
      return livraison.status !== "ramasse";
    }
    return false;
  };
  ...
  if (!livraison) {
    return (
      <div className="py-8 text-center">
        ...
        <h3 className="mt-2 text-sm font-medium text-gray-900">Livraison non trouvée</h3>
        <p className="mt-1 text-sm text-gray-500">La livraison demandée n'existe pas ou a été supprimée.</p>
        <button onClick={() => navigate("/livraisons")} className="px-4 py-2 mt-4 text-primary-600 hover:text-primary-800">
          Retour à la liste
        </button>
      </div>
    );
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            <h1 className="text-2xl font-bold text-gray-900">Livraison #{livraison.id}</h1>
            ...
            {isDepotClient && (
              <div className="flex items-center gap-2 px-3 py-1 text-sm font-medium text-blue-700 bg-blue-100 rounded-full">
          ...
          <p className="mt-2 text-gray-600">{livraison.demande_livraison?.colis?.colis_label || "Sans référence"}</p>

          {(wilayaDepart || wilayaArrivee) && (
            <div className="flex items-center gap-3 mt-3 text-sm text-gray-700">
              <FaMapMarkerAlt className="w-4 h-4 text-gray-400" />
              <div className="flex items-center gap-2">
                {wilayaDepart && <span className="font-medium text-blue-600">{wilayaDepart}</span>}
                {wilayaDepart && wilayaArrivee && <FaArrowLeft className="w-3 h-3 text-gray-400 rotate-180" />}
                {wilayaArrivee && <span className="font-medium text-green-600">{wilayaArrivee}</span>}
              </div>
            </div>
          )}
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <button onClick={handlePrint} disabled={isPrinting} className="flex items-center gap-2 px-4 py-2 transition-colors border border-gray-300 rounded-lg hover:bg-gray-50 disabled:opacity-50">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-600">
                            {livraison.livreur_ramasseur ? `${livraison.livreur_ramasseur.user?.nom || ""} ${livraison.livreur_ramasseur.user?.prenom || ""}`.trim() : "Non attribué"}
                          </p>
                        </div>
                      </div>
                      {canAssignLivreur(livraison, "ramasseur") && (
                        <button onClick={() => openAssignModal("ramasseur")} className="px-3 py-1 text-sm text-blue-600 transition-colors bg-blue-100 rounded hover:bg-blue-200">
                  ...
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-600">
                          {livraison.livreur_distributeur ? `${livraison.livreur_distributeur.user?.nom || ""} ${livraison.livreur_distributeur.user?.prenom || ""}`.trim() : "Non attribué"}
                        </p>
                      </div>
                    </div>
                    {canAssignLivreur(livraison, "distributeur") && (
                      <button onClick={() => openAssignModal("distributeur")} className="px-3 py-1 text-sm text-green-600 transition-colors bg-green-100 rounded hover:bg-green-200">
                        {livraison.livreur_distributeur ? "Changer" : "Attribuer"}
                      </button>
                    )}
                  </div>
          ...
          <div className="p-6 bg-white shadow-sm rounded-xl">
            ...
            <div className={`p-4 rounded-lg mb-6 ${statusInfo.color}`}>
              <div className="flex items-center">
                ...
                <div>
                  <p className="text-lg font-bold capitalize">{statusInfo.label}</p>
            ...
            {!isAnnule && !isLivre && (
              <div>
                <h3 className="mb-3 text-sm font-medium text-gray-900">Actions disponibles</h3>
                <div className="flex flex-wrap gap-2">
                  <button onClick={() => { if (window.confirm("Êtes-vous sûr de vouloir annuler cette livraison ?")) updateStatus("annule"); }} className="px-4 py-2 rounded-lg flex items-center gap-2 bg-red-50 text-red-700 hover:bg-red-100">
                    <FaTimesCircle className="w-4 h-4" /> Marquer comme annulé
                  </button>
                </div>
              </div>
            )}
            ...
            {isAnnule && (
              <div className="p-4 rounded-lg bg-red-50">
                <div className="flex items-center gap-3"><FaBan className="w-6 h-6 text-red-600" /><div><p className="font-medium text-red-800">Livraison annulée</p><p className="text-sm text-red-600">Aucune modification possible.</p></div></div>
              </div>
            )}

            {isLivre && (
              <div className="p-4 rounded-lg bg-green-50">
                <div className="flex items-center gap-3"><FaCheckCircle className="w-6 h-6 text-green-600" /><div><p className="font-medium text-green-800">Livraison terminée</p><p className="text-sm text-green-600">Aucune modification possible.</p></div></div>
              </div>
            )}
          </div>
          ...
          <div className="p-6 bg-white shadow-sm rounded-xl">
            ...
            {livraison.demande_livraison?.info_additionnel && (
              <div className="p-3 mt-4 rounded-lg bg-yellow-50">
                <div className="flex items-center gap-2 mb-1"><FaInfoCircle className="text-yellow-600" /><span className="text-sm font-medium text-yellow-800">Instructions spéciales</span></div>
                <p className="text-yellow-900">{livraison.demande_livraison.info_additionnel}</p>
              </div>
            )}
          </div>
        </div>
          ...
          {isAdmin && (
            <div className="p-6 bg-white shadow-sm rounded-xl">
              ...
              <div className="space-y-3">
                ...
                {isLivre && (<div className="p-3 bg-gray-100 rounded-lg"><p className="text-sm text-center text-gray-600">Livraison terminée - Aucune action disponible</p></div>)}
              </div>
            </div>
          )}
        </div>
