// src/pages/Navettes/components/WilayasTransitSelect.jsx
import React from "react";
import { FaMapMarkerAlt, FaTimes } from "react-icons/fa";

const WilayasTransitSelect = ({ wilayas, selected, onChange, placeholder = "Sélectionner des wilayas de transit" }) => {
  const wilayasList = [
  ...
  const removeWilaya = (codeToRemove) => {
    onChange(selected.filter(code => code !== codeToRemove));
  };

  const getWilayaName = (code) => {
    const wilaya = wilayasList.find(w => w.code === code);
  ...
  return (
    <div className="space-y-3">
      ...
      <select
        ...
        className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-primary-500 focus:border-primary-500 min-h-[120px]"
        size={4}
      >
      ...
      <p className="text-xs text-gray-500">
        Maintenez Ctrl (ou Cmd sur Mac) pour sélectionner plusieurs wilayas
      </p>

      {/* Liste des wilayas sélectionnées */}
      {selected.length > 0 && (
        <div className="mt-3">
          <label className="block text-sm font-medium text-gray-700 mb-2">
            Wilayas de transit sélectionnées ({selected.length}) :
          </label>
...
export default WilayasTransitSelect;